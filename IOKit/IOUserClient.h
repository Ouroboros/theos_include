...
{
    /* 0x00 */ void*    cleartext;
    /* 0x08 */ void*    ciphertext;
    /* 0x10 */ uint32_t size;
    /* 0x14 */ uint8_t  iv[0x10];
    /* 0x24 */ uint32_t mode;
    /* 0x28 */ uint32_t bits;
    /* 0x2C */ uint8_t  keybuf[0x20];
    /* 0x4C */ uint32_t mask;
    /* 0x50 */ uint32_t length_of_uidplus_params; //ios 4.2.1
    /* 0x54 */ IOAES_UIDPlus_Params uidplus_params;
...
struct IOExternalMethodArguments
{
    ...
    const void *	structureInput;                     // 0x30
    uint32_t		structureInputSize;                 // 0x38

    IOMemoryDescriptor * structureInputDescriptor;      // 0x40

    uint64_t *		scalarOutput;                       // 0x48
    uint32_t		scalarOutputCount;                  // 0x50

    void *		structureOutput;                        // 0x58
    uint32_t		structureOutputSize;                // 0x60
